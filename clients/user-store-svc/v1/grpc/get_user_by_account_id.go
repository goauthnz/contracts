...
func (c *UserStoreSvcClient) GetUserByAccountID(ctx context.Context, accountID string) (*user_store_svc_v1_entities.User, error) {
	req := connect.NewRequest(&pb.GetUserByAccountIDRequest{
		AccountId: wrapperspb.String(accountID),
	})

	res, err := c.client.GetUserByAccountID(ctx, req)
	if err != nil {
