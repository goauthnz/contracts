...
func (c *AuthnzStoreSvcClient) CreateAccountWithEmail(ctx context.Context, email string) (*authnz_store_svc_v1_entities.Account, string, error) {
	req := connect.NewRequest(&pb.CreateAccountWithEmailRequest{
		Email: wrapperspb.String(email),
	})

	res, err := c.client.CreateAccountWithEmail(ctx, req)
	if err != nil {
