...
func (c *AuthnzStoreSvcClient) GetSessionByToken(ctx context.Context, token string) (*authnz_store_svc_v1_entities.Session, error) {
	req := connect.NewRequest(&pb.GetSessionByTokenRequest{
		Token: wrapperspb.String(token),
	})

	res, err := c.client.GetSessionByToken(ctx, req)
	if err != nil {
