...
func (c *AuthnzStoreSvcClient) GetAccountByEmail(ctx context.Context, email string) (*authnz_store_svc_v1_entities.Account, error) {
	req := connect.NewRequest(&pb.GetAccountByEmailRequest{
		Email: wrapperspb.String(email),
	})

	res, err := c.client.GetAccountByEmail(ctx, req)
	if err != nil {
