...
func (c *AuthnzStoreSvcClient) CreateAccountWithPhone(ctx context.Context, phone string) (*authnz_store_svc_v1_entities.Account, string, error) {
	req := connect.NewRequest(&pb.CreateAccountWithPhoneRequest{
		Phone: wrapperspb.String(phone),
	})

	res, err := c.client.CreateAccountWithPhone(ctx, req)
	if err != nil {
