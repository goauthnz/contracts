...
func (c *AuthnzStoreSvcClient) GetAccountByIdentityID(ctx context.Context, identityID string) (*authnz_store_svc_v1_entities.Account, error) {
	req := connect.NewRequest(&pb.GetAccountByIdentityIDRequest{
		IdentityId: wrapperspb.String(identityID),
	})

	res, err := c.client.GetAccountByIdentityID(ctx, req)
	if err != nil {
