...
func (c *AuthnzStoreSvcClient) CreateAccountWithPhonePassword(ctx context.Context, phone string, password string) (*authnz_store_svc_v1_entities.Account, string, error) {
	req := connect.NewRequest(&pb.CreateAccountWithPhonePasswordRequest{
		Phone:    wrapperspb.String(phone),
		Password: wrapperspb.String(password),
	})

	res, err := c.client.CreateAccountWithPhonePassword(ctx, req)
	if err != nil {
