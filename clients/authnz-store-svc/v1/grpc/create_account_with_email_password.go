...
func (c *AuthnzStoreSvcClient) CreateAccountWithEmailPassword(ctx context.Context, email string, password string) (*authnz_store_svc_v1_entities.Account, string, error) {
	req := connect.NewRequest(&pb.CreateAccountWithEmailPasswordRequest{
		Email:    wrapperspb.String(email),
		Password: wrapperspb.String(password),
	})

	res, err := c.client.CreateAccountWithEmailPassword(ctx, req)
	if err != nil {
	...
	}, res.Msg.GetVerificationCode().GetValue(), nil
}
