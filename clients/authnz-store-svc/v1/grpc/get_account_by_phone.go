...
func (c *AuthnzStoreSvcClient) GetAccountByPhone(ctx context.Context, phone string) (*authnz_store_svc_v1_entities.Account, error) {
	req := connect.NewRequest(&pb.GetAccountByPhoneRequest{
		Phone: wrapperspb.String(phone),
	})

	res, err := c.client.GetAccountByPhone(ctx, req)
	if err != nil {
